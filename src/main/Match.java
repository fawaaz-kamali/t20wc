...
public class Match
{
    ...
    private static void battingCard()
    {
         ...
         */
        System.out.println("============================================");
        System.out.printf("%-40s%-10s%-10s%-15s%-10s%-20s%n", "Name", "Runs", "Balls", "Strike Rate", "Batting%", "Overall Rating");
    ...
    {
        System.out.println("============================================");
        System.out.printf("%-40s%-10s%-10s%-10s%-10s%-10s%s %n", "Name", "Overs", "Wickets", "Runs", "Economy", "Bowling%", "Overall Rating");
        for (int i = 0; i < availableBowlers.size(); i++)
        {
            ...
            System.out.printf("%-40s%s.%-8s%-10s%-10s%-10.2f%-10s%-10s %n", p.getName(), p.getMatchBallsBowled()/6, p.getMatchBallsBowled()%6, p.getMatchWickets(), p.getMatchRunsConceded(), economy , p.getBowlingRating(), p.getOverallRating());
        }
        System.out.println("============================================");
        System.out.print("Press the enter key to advance> ");
        s.nextLine();
    ...
    private static Player pickBowler(Player previousBowler)
    {
        ...
        {
            System.out.println("============================================");
            System.out.printf("%-10s%-40s%-10s%-10s%-10s%-10s%-10s%s %n", "No.", "Name", "Overs", "Wickets", "Runs", "Economy", "Bowling%", "Overall Rating");
            for (int i = 0; i < availableBowlers.size(); i++)
            {
                Player p = availableBowlers.get(i);
                double economy = 0.00;
                ...
                System.out.printf("%-10s%-40s%s.%-8s%-10s%-10s%-10.2f%-10s%-10s %n", i+1, p.getName(), p.getMatchBallsBowled()/6, p.getMatchBallsBowled()%6, p.getMatchWickets(), p.getMatchRunsConceded(), economy , p.getBowlingRating(), p.getOverallRating());
            }
            System.out.println("============================================");
            ...
            {
                System.out.println("The same bowler can not bowl a second consecutive over. You must pick another bowler.");
            }
